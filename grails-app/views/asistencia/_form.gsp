...
<div class="fieldcontain ${hasErrors(bean: asistenciaInstance, field: 'persona', 'error')} required form-group">
  ...
  <div class="col-lg-3">
	<g:select id="persona" name="persona.id" from="${salias.Persona.get(asistenciaInstance?.persona?.id)}" optionKey="id" required="" value="${asistenciaInstance?.persona?.id}" class="form-control"/>
</div>
  </div>
